...
class ShuffleNetV2(nn.Module):
    def __init__(self, planes, layers, groups, num_classes=2):
        super(ShuffleNetV2, self).__init__()
        self.groups = groups
        self.stage1 = nn.Sequential(
            Conv3x3BNReLU(in_channels=55, out_channels=24, stride=2, groups=1),
        ...
        self.info_conv1 = nn.Linear(5, 256)  # 5个临床特征，上采样为512
        self.linear = nn.Linear(in_features=planes[2] + 256, out_features=num_classes)
        self.sigmoid = nn.Sigmoid()
        # self.sigmoid = nn.Softmax()  # 激活函数
        self.init_params()
    ...
    def forward(self, x, info):
        ...
        info = self.info_conv1(info)  # 5个临床特征，上采样为512
